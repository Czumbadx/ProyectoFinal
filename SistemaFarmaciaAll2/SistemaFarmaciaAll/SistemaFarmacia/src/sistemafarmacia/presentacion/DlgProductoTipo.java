...
public class DlgProductoTipo extends javax.swing.JDialog {

    private static final String TITLE = "Tipos de Productos";
    VariablesFunciones variables = new VariablesFunciones();
    ProductoTipoDAT datos = new ProductoTipoDAT();
    static int idProductoTipo;

    
    public DlgProductoTipo(java.awt.Frame parent, boolean modal, int devuelve) {
        super(parent, modal);
        initComponents();
        ...
        llenarLista();
        controlFunciones(1);
        idProductoTipo = devuelve; 
    }
    
    private void llenarLista(){
        DefaultTableModel dtm = new DefaultTableModel();
        variables.limpiarDefaulTableModel(dtm);
        String cabecera[] = {"NOMBRE","DESCRIPCION","ESTADO","ESTADO_ID","ID"};
        dtm.setColumnIdentifiers(cabecera);
        tbLista.setModel(dtm);
        try {
            ArrayList<ProductoTipo> lista = datos.getLista(txtBuscar.getText(),2,0);/*all={2{todos},1{activos},0{inactivos}},limit ={0{todos},n{limite}}*/
            String datos[] = new String[5];
            
            for (int i = 0; i<lista.size(); i++) {
                datos[0] = String.valueOf(lista.get(i).getNombre());
                datos[1] = String.valueOf(lista.get(i).getDescripcion());
                datos[2] = String.valueOf(lista.get(i).getEstadoDescripcion());
                datos[3] = String.valueOf(lista.get(i).getEstado());
                datos[4] = String.valueOf(lista.get(i).getId());
                dtm.addRow(datos);               
            }
    ...
    private void insert(){
        try {
            int res = datos.insert(new ProductoTipo(0,txtNombre.getText(),txtDescripcion.getText(),0));
            if(res>0) {
                if(idProductoTipo>0) idProductoTipo = res;
                llenarLista();
                controlFunciones(1);
            }else{
    ...
    private void update(){
        try {
            int estado = (optActivo.isSelected()) ? 1 : 0 ; 
            int res = datos.update(
                new ProductoTipo(Integer.parseInt(txtId.getText()),txtNombre.getText(),txtDescripcion.getText(),estado)
            );
            if(res>0) {
                if(idProductoTipo>0) idProductoTipo = res;
                llenarLista();
    ...
    private void mostrarDatosTabla(){
        ...
        txtDescripcion.setText(tbm.getValueAt(tbLista.getSelectedRow(), 1).toString());
    ...
    private void controlBotones(boolean activo, boolean estado, String guardar){
        ...
        txtDescripcion.setEditable(activo);
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Buscar Tipos"));
...
controlFunciones(1);
    ...
    public void setIdProductoTipo(int idProductoTipo) {
        DlgProductoTipo.idProductoTipo = idProductoTipo;
    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DlgProductoTipo.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                DlgProductoTipo dialog = new DlgProductoTipo(new javax.swing.JFrame(), true,idProductoTipo);
